...
TEST_CASE("Test Random") {
  ...
  std::uniform_int_distribution<int> rand4 {0, 15};
  std::uniform_int_distribution<uint16_t> rand16;

  model.a = static_cast<uint8_t>(rand4(re));
  model.b = static_cast<uint16_t>(rand16(re));
  model.c = static_cast<uint16_t>(rand16(re));
  model.nReset = 0;
  step(model);
